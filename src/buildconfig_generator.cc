#include "buildconfig_generator.h"she
#include "util.h"
#include <assert.h>
#include <dirent.h>
#include <stdio.h>
...
int ConfigGenerator::is_head_file(const char* file) {
    if(strstr(file, ".h") != 0 ||
       strstr(file, ".hh") != 0 ||
       strstr(file, ".hpp") != 0) {
        return 1;
...
int ConfigGenerator::get_more_line(fstream& stream, string& res) {
    string line;
    size_t comment = 0;
    size_t begin = 0;
    size_t end = 0;
...
int ConfigGenerator::is_main_file(const char* file) {
...
FileType ConfigGenerator::Get_FileType(const char* name) {
...
int ConfigGenerator::Load_files_iter(const char* path) {
...
string ConfigGenerator::Get_obj_name(const string& name, string end) {
    size_t pos;
    string res = name;
    pos = name.rfind("/");
    if(pos != string::npos) {
...
int ConfigGenerator::Write_config_file() {
...
int ConfigGenerator::Init_LoadConfig(std::string path) {
...
int ConfigGenerator::Fresh_LoadConfig(std::string path) {
    return 0;
}
